...
Thunder provides the [@thunder-so/thunder](https://github.com/thunder-so/thunder) package for deploying Lambda functions with API Gateway, supporting both standard zip deployments and container images for custom runtimes.
...
## Serverless API Hosting Architecture
...
Get your serverless API deployed to AWS Lambda in minutes. This guide covers installation, basic configuration, and deployment with the `@thunder-so/thunder` package.
...
const fnProps: LambdaProps = {
  ...
  functionProps: {
    dockerFile: 'Dockerfile',  // required for container pipeline
  },
  buildProps: {
    installcmd: 'bun install',
    buildcmd: 'bun run build',
