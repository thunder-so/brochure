...
Thunder provides a secure and battle-tested pattern for deploying any SPA/SSG on AWS infrastructure using [AWS CDK](https://aws.amazon.com/cdk/). This pattern leverages the [@thunder-so/thunder](https://github.com/thunder-so/thunder) package, which implements best practices for static site hosting with CloudFront distribution, S3 origin, and Lambda@Edge functions for advanced routing capabilities.
...
Get your static site deployed to AWS in minutes with the `@thunder-so/thunder` package. This guide walks through installation, configuration, and deployment of a basic static site.
