...
1. **Static Site Generation (SSG)** — Deploy static Analog sites using S3 and CloudFront with the `Static` construct
2. **Serverless SSR** — Deploy SSR Analog applications using Lambda, S3, and CloudFront with the `AnalogJS` construct
3. **Containerized SSR** — Deploy SSR Analog applications using ECS Fargate with the `Fargate` construct
